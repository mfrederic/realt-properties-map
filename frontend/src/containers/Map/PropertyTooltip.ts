...
export function propertyTooltip({
  ...
  return `
  <div>
    ${ ownedAmount <= 0 ? `<div class="px-2 py-2 font-bold text-center text-red-500">Not owned</div>` : ''}
      ...
      <dl class="divide-y divide-gray-100">
        ${
          items.map((item, index) => (
            `<div key=${index} class="px-2 py-2 grid grid-cols-12">
              <dt class="text-base font-medium leading-6 text-gray-900 col-span-5">${ item.label }</dt>
              <dd class="text-base mt-1 text-right leading-6 text-gray-700 col-span-7 mt-0">${ item.value }</dd>
            </div>`
          )).join('')
        }
      </dl>
    ...
    ${
      actions ? 
      `<div class="px-2 py-2 grid grid-cols-12">${actions}</div>` : ''
    }
  </div>
  `;
}